...
flowchart LR
  ...
  CMD --> MODEL["Schema 1 project model"]
...
React owns navigation and editor state. All persistent operations cross typed Tauri commands. The main window hosts Home, setup, and the editor. The HUD and region selector are separate always-on-top webviews protected from capture.
...
`ProjectManifest` is the source of truth. Schema `1` is camel-cased at the JavaScript boundary and contains ordered steps with before/after media references. Active work is atomically saved under `%LOCALAPPDATA%\Crumbtrail\sessions\<project-id>`.

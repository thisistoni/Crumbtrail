...
- Application/control names and UI Automation identifiers when Windows exposes them.
...
Active sessions live under `%LOCALAPPDATA%\Crumbtrail\sessions`. A completed edit is written to a temporary file and atomically moved into place. Session folders are intentionally retained for Home-screen recovery until the user removes them.
